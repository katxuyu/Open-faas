...
def handle(event, context):
    request = event
    if "/v1/packetview/" in str(event.path):
        key = str(event.path).split("/")[-1]
        if request.method == 'GET':
          return {
                      "statusCode": 405,
                      "body": "You performed a GET request with this key: " + key +".\n You need to do a POST request with a JSON payload"
                  }
        else:
          payload = {"key" : key, "payload" : json.loads((request.body).decode('utf-8'))}
          send_payload_to_kafka("packetview", 'TTI', payload)
          return {
                      "statusCode": 200,
                      "body": {"message": "OK"},
                      "headers": {
        ...
        if request.method == 'GET':
            ...
            if str(request.path) == "/v1/inmarsat":
                application_id = payload["application_id"]
            elif str(request.path) == "/v1/payloads":
                application_id = payload["end_device_ids"]["application_ids"]["application_id"]
            elif str(request.path) == "/v1/ptdata":
                application_id = payload["end_device_ids"]["application_ids"]["application_id"]
            else:
                return {
                    "statusCode": 404,
                    "body": f"Path Not Found: {request.path}"
                }

            if authorized(request.headers, application_id):
                if str(request.path) == "/v1/inmarsat":
                    send_payload_to_kafka("pwxpayloads", 'INMARSAT', payload)
                elif str(request.path) == "/v1/payloads":
                    send_payload_to_kafka("pwxpayloads", 'TTI', payload)
                elif str(request.path) == "/v1/ptdata":
